...
class InferenceXArmTCP:
    def __init__(
        ...
        self.action_type = cd["data"]["train"].get("action_type", "absolute")
        ...
        self.act_min = torch.tensor(stats[action_stat_name]["q01"]).unsqueeze(0).to(self.device, dtype=self.dtype)
        self.act_max = torch.tensor(stats[action_stat_name]["q99"]).unsqueeze(0).to(self.device, dtype=self.dtype)
        self.states_min = torch.tensor(stats[state_stat_name]["q01"]).unsqueeze(0).to(self.device, dtype=self.dtype)
        self.states_max = torch.tensor(stats[state_stat_name]["q99"]).unsqueeze(0).to(self.device, dtype=self.dtype)
    ...
    def play(self, obs, prompt, execution_step=1, state=None):
        ...
        actions_pred = pred_all["action"].detach().to(self.device, dtype=self.dtype)[0]
        
        # Relative action handling: action = model_pred + current_state (both in normed space)
        if self.action_type == "relative" and history_action_state is not None:
            # history_action_state is [1, 1, 14] where [7:14] is normed state
            normed_state = history_action_state[0, 0, 7:14]
            actions_pred[:, :7] = actions_pred[:, :7] + normed_state
...
def main():
    ...
    # TCP specific: Angle history for unwrapping
    code, current_tcp_raw = arm.get_position(is_radian=True)
    current_tcp = np.array(current_tcp_raw[:6])
    angle_history = deque(maxlen=2)
    angle_history.append(current_tcp[3:].copy())
    last_target_tcp = current_tcp.copy()
    ...
    try:
        while kb.running:
            ...
            # TCP State Acquisition with Unwrapping
            ...
            cv2.putText(pane3, f"TCP Pose:", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
            y_offset += 15
            labels = ['X', 'Y', 'Z', 'R', 'P', 'Y']
            for idx, label in enumerate(labels):
                val = raw_tcp[idx]
                val_str = f"{val:.2f}" if idx < 3 else f"{np.rad2deg(val):.1f}"
                cv2.putText(pane3, f" {label}: {val_str}", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
                y_offset += 15
            ...
            cv2.imwrite("eval_xarm_tcp_input.jpg", debug_img)

            # Norm state and pad with zeros for [action(7), state(7)] format
            state_tensor = torch.tensor(current_state).unsqueeze(0).to(infer.device, dtype=infer.dtype)
            state_normed = (state_tensor - infer.states_min) / (infer.states_max - infer.states_min + 1e-6)
            state_normed = state_normed * 2 - 1
            # Padding: [zeros(7), state_normed(7)]
            state_input = torch.cat((torch.zeros_like(state_normed), state_normed), dim=1)

            actions = infer.play(stacked_obs, args.prompt, execution_step=args.exec_step, state=state_input)
            ...
            for i in range(args.exec_step):
                ...
                # Update current TCP for smoothing/safety
                ...
                if code_now == 0:
                    ...
                    last_target_tcp = raw_now

                # Ensure shortest path for orientation
                angle_diff = target_tcp[3:] - last_target_tcp[3:]
                angle_diff = (angle_diff + np.pi) % (2 * np.pi) - np.pi
                target_tcp[3:] = last_target_tcp[3:] + angle_diff

                # Safety Clip
                if np.any(np.abs(target_tcp[:3] - last_target_tcp[:3]) > 50.0):
                    target_tcp[:3] = np.clip(target_tcp[:3], last_target_tcp[:3] - 10.0, last_target_tcp[:3] + 10.0)
                if np.any(np.abs(target_tcp[3:] - last_target_tcp[3:]) > 0.4):
                    target_tcp[3:] = np.clip(target_tcp[3:], last_target_tcp[3:] - 0.1, last_target_tcp[3:] + 0.1)

                # Smoothing
                executed_tcp = (1 - args.smoothing) * last_target_tcp + args.smoothing * target_tcp
